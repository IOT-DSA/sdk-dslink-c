...
void dslink_node_tree_free_basic(DSLink *link, DSNode *root) {
    ...
    // TODO: remove node from open_streams, list_subs, and value_path_subs: implemented
    dslink_map_remove(link->responder->value_path_subs, root->path);
    ...
    ref_t* ridRef = dslink_map_remove_get(link->responder->list_subs,root->path);
    if(ridRef)
